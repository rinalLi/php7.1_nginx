...
RUN apt-get update && apt-get upgrade -y && \ 
    apt-get install -y --no-install-recommends apt-utils \
	    ...
	    LC_ALL=en_US.UTF-8 add-apt-repository ppa:ondrej/nginx
...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y curl build-essential \
    ...
    python-pip re2c unattended-upgrades whois vim libnotify-bin wget \
    debconf-utils
...
# install php
RUN apt-get install -y --force-yes php7.1-fpm php7.1-cli php7.1-dev php7.1-sqlite3 php7.1-gd php-mongodb \
